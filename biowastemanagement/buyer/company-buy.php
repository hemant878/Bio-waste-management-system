...
<?php
$s="SELECT * FROM `form` WHERE `equipment`='Needles' OR `equipment`='Burner' OR `equipment`='Scalpels' OR `equipment`='Blades' OR `equipment`='Discarded Metal Sharp' OR `equipment`='Cutter'OR `equipment`='Lancets' ";
$re=$connect->query($s);
$connect->close();
?>
...
<ul class="nav nav-tabs nav-fill">
    ...
    <li class="nav-item">
        <a class="nav-link active text-dark" aria-current="page" href="#">White Waste</a>
    </li>
    <li class="nav-item">
...
<center><h2 style="margin: 10px 0 10px 0"> White Category Bio-Medical Waste  </h2></center>
<form method="POST" action="company-form.php">
<button type="submit" class="btn btn-dark my-2 " style="margin:0px 0 0 20px" > Buying Form</button>
</form>
<table class="table table-striped"   > 
            ...
            while($rows=$re->fetch_assoc()){  ?>
                <tr>
                <td class="aas"><?php echo $rows['equipment'];?></td> 
          <!--      <td class=""><?php //echo $rows['quantity'];?></td> -->
                <td class=""><?php echo $rows['weight']." kg";?></td>
                <td class=""><?php echo $rows['p_description'];?></td> 
                <td class=""><?php echo $rows['p_condition'];?></td> 
